...
// Serialplot发送缓冲区
static uint8_t Serialplot_Tx_Buffer[UART_BUFFER_SIZE];
...
/**
 * @brief 简易字符串转float, 解析形如-12.34的十进制数
 */
static float Serialplot_Parse_Float(const uint8_t *Data, uint16_t Start, uint16_t End)
{
    ...
    for (; i < End; i++)
    {
        ...
        {
            value += (float)(ch - '0') * decimal_factor;
            decimal_factor *= 0.1f;
        }
    }
...
int32_t Class_Serialplot_UART::Judge_Variable_Name(const uint8_t *Rx_Data, uint16_t Length, uint16_t *value_start_index)
{
    if ((Rx_Data == nullptr) ||
        (Length == 0) ||
        (value_start_index == nullptr) ||
        (Rx_Variable_List == nullptr) ||
        (Rx_Variable_Num == 0))
    {
        return (-1);
    }
 ...
 * @brief 获取数据类型大小
 */
uint16_t Class_Serialplot_UART::Get_Data_Type_Size()
{
    switch (Tx_Data_Type)
    {
