...
public:
     ...
     */
    inline int32_t Get_Variable_Index();
 ...
 * @brief 获取当前接收指令在字典中的编号
 */
inline int32_t Class_Serialplot_UART::Get_Variable_Index()
{
 ...
 * @brief 获取当前接收指令的值
 */
inline float Class_Serialplot_UART::Get_Variable_Value()
{
...
/**
 * @brief 绑定需要发送的变量地址
 * @note 这里保存的是变量地址, 不是变量当前值。后续每次调用TIM_1ms_Write_PeriodElapsedCallback时, 才会读取这些地址里的最新值。
 */
